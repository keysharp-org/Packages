/*
    FindText for Keysharp — cross-platform port of FeiYue's FindText.ahk (v10.2).

    Original : FindText by FeiYue, https://www.autohotkey.com/boards/viewtopic.php?f=83&t=116471
    Port     : this file is FindText.ahk, COPIED, with only the Windows-only calls replaced.

    THE PORTING RULE, AND WHY IT MATTERS
        Everything here is FeiYue's code verbatim unless a comment says otherwise. The library is
        thousands of lines of behaviour debugged over years against real screens; rewriting any of
        it re-derives that badly. So the diff against dist/FindText.ahk is deliberately small and
        auditable, and confined to the places that genuinely cannot work off Windows:

          1. The matcher was x86/x64 machine code pasted in as base64 and run via
             DllCall(VirtualProtect(...)). The C it was compiled from shipped in the library's own
             code() method; the `#CSharp` block below is a transcription of exactly that C, so
             Roslyn compiles it into the script's own assembly instead.
          2. Screen capture and pixel access went through GDI (CreateDIBSection / BitBlt / GetDCEx /
             LoadPicture / DrawHBM / ...). Those become the KS Image class.

        The 1264-line Gui() state machine contained just two DllCalls (DeleteObject on an HBITMAP),
        so it is copied unchanged. Same for Lang(), GetRange(), ObjView(), the Ocr/Sort helpers and
        the rest of the interaction layer.

    This file uses #CSharp, so it is a Keysharp script, not an AutoHotkey one.
*/
...
// Channel offsets inside one pixel. Image.GetPixelData(4) hands back R,G,B,A; the original
// walked a 32bpp Windows DIB, i.e. B,G,R,A. This is the entire difference between the two.
// (Named rather than inlined so every read site shows which channel it means.)

// The parameter list is FindText.ahk's DllCall argument list, in the same order and with the same
// meanings -- addresses stay addresses -- so PicFind() below is a one-line swap of the DllCall
// rather than a rewrite of its caller. Script locals are kept alive for their whole scope by the
// compiler, so a Buffer whose address is passed here cannot be collected mid-match.
[Keysharp.Runtime.Static] public static unsafe long PicFind(object @this,
    long modeArg, long colorArg, long nArg, long dirArg,
    long bmpPtr, long strideArg, long zwArg, long zhArg,
    long sxArg, long syArg, long swArg, long shArg,
    long ssPtr, long s1Ptr, long s0Ptr,
    long textPtr, long wArg, long hArg,
    long err1Arg, long err0Arg, long moreErrArg,
    long allposPtr, long allposMaxArg,
    long newWArg, long newHArg)
{
    const int R = 0, G = 1, B = 2;

    int mode = (int)modeArg, dir = (int)dirArg;
    int Stride = (int)strideArg, zw = (int)zwArg, zh = (int)zhArg;
    int sx = (int)sxArg, sy = (int)syArg, sw = (int)swArg, sh = (int)shArg;
    int w = (int)wArg, h = (int)hArg, new_w = (int)newWArg, new_h = (int)newHArg;
    int err1 = (int)err1Arg, err0 = (int)err0Arg, more_err = (int)moreErrArg;
    int allpos_max = (int)allposMaxArg;
    uint c = (uint)colorArg, n = (uint)nArg;

    byte* Bmp = (byte*)bmpPtr;
    byte* text = (byte*)textPtr;
    // `cors` is derived from `text`, exactly as the C did: the color table sits directly after the
    // pattern data in one allocation, which is the layout PicInfo() still builds.
    uint* cors = null;
    uint* s1 = (uint*)s1Ptr;
    uint* s0 = (uint*)s0Ptr;
    uint* allpos = (uint*)allposPtr;
    uint* arr = allpos + allpos_max;
    byte* ss = (byte*)ssPtr;
    byte* ts = ss + sw;
    byte* gs = ts + sw;

    int ok = 0, o = 0, i = 0, j = 0, k = 0, v = 0, t = 0, e1 = 0, e0 = 0;
    int len1 = 0, len0 = 0, max = 0, pic = 0, shape = 0, dir1 = 0;
    int x = 0, y = 0, x1 = 0, y1 = 0, x2 = 0, y2 = 0, x3 = 0, y3 = 0;
    int r = 0, g = 0, b = 0, rr = 0, gg = 0, bb = 0, dR = 0, dG = 0, dB = 0;
    int ii = 0, jj = 0, RunDir = 0, DirCount = 1, RunCount = 0, AllCount1 = 0, AllCount2 = 0;
    uint c1 = 0, c2 = 0;
    bool resume = false;
    ...
    if (mode == 5)
    {
        ...
        if (pic != 0)  // FindPic
        {
            cors = (uint*)(text + (long)w * h * 4); j = (err0 >> 10) + 1; n *= 2; k = (n <= 2 || cors[2] < 0x1000000) ? 1 : 0;
            c2 = cors[1]; r = (int)((c2 >> 16) & 0xFF); g = (int)((c2 >> 8) & 0xFF); b = (int)(c2 & 0xFF);
            dR = r * r; dG = g * g; dB = b * b;
            for (y = 0; y < h; y += j)
            for (x = 0; x < w; x += j)
            {
                o = (y * w + x) * 4; rr = text[o + R]; gg = text[o + G]; bb = text[o + B];
                for (i = 2; i < n;)
                {
                    c1 = cors[i++]; c2 = cors[i++];
                    r = (int)((c1 >> 16) & 0xFF) - rr; g = (int)((c1 >> 8) & 0xFF) - gg; b = (int)(c1 & 0xFF) - bb;
                    v = (c2 < 0x40000000)
                      ? (((1024 + (r + rr + rr)) * r * r + 2048 * g * g + (1534 - (r + rr + rr)) * b * b <= c2) ? 1 : 0)
                      : ((r * r <= ((c2 >> 16) & 0xFF) * ((c2 >> 16) & 0xFF)
                       && g * g <= ((c2 >> 8) & 0xFF) * ((c2 >> 8) & 0xFF)
                       && b * b <= (c2 & 0xFF) * (c2 & 0xFF)) ? 1 : 0);
                    if (v != 0) break;
                }
                if (v == k) continue;
                s1[len1] = (uint)((y * new_h / h) * Stride + (x * new_w / w) * 4);
                s0[len1++] = (uint)(rr << 16 | gg << 8 | bb);
            }
        }
        else  // FindMultiColor or FindColor
        {
            shape = (c == 1) ? 1 : 0;  // FindShape
            cors = (uint*)text;
            for (i = 0; i < n; i++, o += 22)
            {
                c = cors[o]; y = (int)(c >> 16); x = (int)(c & 0xFFFF);
                s1[len1] = (uint)((y * new_h / h) * Stride + (x * new_w / w) * 4);
                s0[len1++] = (uint)(o + cors[o + 1] * 2);
            }
            cors += 2;
        }
        goto StartLookUp;
    ...
    for (y = 0; y < h; y++)
    {
        for (x = 0; x < w; x++)
        {
            i = (mode == 4) ? (y * new_h / h) * Stride + (x * new_w / w) * 4
                            : (y * new_h / h) * sw + (x * new_w / w);
            if (text[o++] == (byte)'1')
                s1[len1++] = (uint)i;
            else
                s0[len0++] = (uint)i;
    ...
    if (mode == 4)
    {
        y = (int)(c >> 16); x = (int)(c & 0xFFFF);
        c = (uint)((y * new_h / h) * Stride + (x * new_w / w) * 4);
        goto StartLookUp;
    }
    //----------------------
    ...
    {
        cors = (uint*)(text + (long)w * h); n *= 2; k = (cors[0] < 0x1000000) ? 1 : 0;
        for (y = 0; y < sh; y++, o += j)
        for (x = 0; x < sw; x++, o += 4, i++)
        {
            rr = Bmp[o + R]; gg = Bmp[o + G]; bb = Bmp[o + B];
            for (t = 0; t < n;)
            {
                c1 = cors[t++]; c2 = cors[t++];
                r = (int)((c1 >> 16) & 0xFF) - rr; g = (int)((c1 >> 8) & 0xFF) - gg; b = (int)(c1 & 0xFF) - bb;
    ...
    else if (mode == 2)  // Gray Threshold Mode
    {
        ...
        for (y = 0; y < sh; y++, o += j)
            for (x = 0; x < sw; x++, o += 4, i++)
                ts[i] = (byte)((Bmp[o + R] * 38 + Bmp[o + G] * 75 + Bmp[o + B] * 15 < c) ? 2 : 1);
    }
    else if (mode == 3)  // Gray Difference Mode
    {
        ...
        for (y = sy - 1; y <= sy + sh; y++, o += j)
        {
            for (x = sx - 1; x <= sx + sw; x++, o += 4, i++)
                gs[i] = (byte)((x < 0 || x >= zw || y < 0 || y >= zh) ? 0
                    : (Bmp[o + R] * 38 + Bmp[o + G] * 75 + Bmp[o + B] * 15) >> 7);
        }
        for (i = 0, y = 1; y <= sh; y++)
        for (x = 1; x <= sw; x++, i++)
        {
            o = y * k + x; n = (uint)(gs[o] + c);
            ts[i] = (byte)((gs[o - 1] > n || gs[o + 1] > n
    ...
    if (more_err != 0)
    {
        for (sx++, i = 0, y = 0; y < sh; y++)
            for (x = 0; x < sw; x++, i++)
                ss[i] = (byte)(ts[i] | (x == 0 ? 0 : ts[i - 1]) | (x == sw - 1 ? 0 : ts[i + 1]));
    }
    else ss = ts;
    //----------------------
    StartLookUp:
    ...
    if (dir == 9)
    {
        ...
        AllCount2 = i * i; RunCount = 0; DirCount = 1; RunDir = 0; ii = 0; jj = 0;
        goto NextSpiral;
    }
    ...
    // Raster scan: one step per entry, cursor kept in x3/y3 so NoMatch can jump back in.
    NextRaster:
    for (;;)
    {
        if (y3 > y2) goto Return1;
        if (x3 > x2) { x3 = x1; y3++; continue; }
        y = ((dir1 & 2) != 0) ? y1 + y2 - y3 : y3;
        x = ((dir1 & 1) != 0) ? x1 + x2 - x3 : x3;
        if (dir1 > 3) { r = y; y = x; x = r; }
        x3++;
        goto FindPos;
    }

    // Spiral scan (dir=9). `resume` is the C's FindPos_GoBak: the step that handed a position to
    // FindPos still owes its move and its jj/ii increment, which is what we do on re-entry.
    NextSpiral:
    for (;;)
    {
        if (!resume)
        {
            if (!(RunCount < AllCount1 && ii < AllCount2)) goto Return1;
            if (jj >= DirCount) { if ((RunDir & 1) != 0) DirCount++; RunDir = (RunDir + 1) & 3; jj = 0; continue; }
            if (x >= x1 && x <= x2 && y >= y1 && y <= y2) { RunCount++; resume = true; goto FindPos; }
        }
        resume = false;
        if (RunDir == 0) y--; else if (RunDir == 1) x++; else if (RunDir == 2) y++; else x--;
        jj++; ii++;
    }
    //----------------------
    FindPos:
    ...
    else if (mode == 5)
    {
        ...
        if (pic != 0)
        {
            for (i = 0; i < max; i++)
            {
                j = (int)(o + s1[i]); c = s0[i]; r = Bmp[j + R] - (int)(c >> 16);
                g = Bmp[j + G] - (int)((c >> 8) & 0xFF); b = Bmp[j + B] - (int)(c & 0xFF);
        ...
        else
        {
            for (i = 0; i < max; i++)
            {
                j = (int)(o + s1[i]); rr = Bmp[j + R]; gg = Bmp[j + G]; bb = Bmp[j + B];
                for (j = i * 22, k = (cors[j] > 0xFFFFFF) ? 1 : 0, n = s0[i]; j < n;)
                {
                    c1 = cors[j++]; c2 = cors[j++];
                    if (shape != 0) { if (i == 0) { c = (uint)(rr << 16 | gg << 8 | bb); goto MatchOK; } c1 = c; }
                    r = (int)((c1 >> 16) & 0xFF) - rr; g = (int)((c1 >> 8) & 0xFF) - gg; b = (int)(c1 & 0xFF) - bb;
                    v = (c2 < 0x40000000)
                      ? (((1024 + (r + rr + rr)) * r * r + 2048 * g * g + (1534 - (r + rr + rr)) * b * b <= c2) ? 1 : 0)
                      : ((r * r <= ((c2 >> 16) & 0xFF) * ((c2 >> 16) & 0xFF)
                       && g * g <= ((c2 >> 8) & 0xFF) * ((c2 >> 8) & 0xFF)
                       && b * b <= (c2 & 0xFF) * (c2 & 0xFF)) ? 1 : 0);
                    if (v != 0) { if (k != 0) goto NoMatch2; goto MatchOK; }
                }
                if (k != 0) goto MatchOK;
                NoMatch2:
    ...
    {
        o = (sy + y) * Stride + (sx + x) * 4;
        j = o + (int)c; rr = Bmp[j + R]; gg = Bmp[j + G]; bb = Bmp[j + B];
        for (i = 0; i < max; i++)
        {
            if (i < len1)
            {
                j = (int)(o + s1[i]); r = Bmp[j + R] - rr; g = Bmp[j + G] - gg; b = Bmp[j + B] - bb;
                if ((1024 + (r + rr + rr)) * r * r + 2048 * g * g + (1534 - (r + rr + rr)) * b * b > n && (--e1) < 0) goto NoMatch;
            }
            if (i < len0)
            {
                j = (int)(o + s0[i]); r = Bmp[j + R] - rr; g = Bmp[j + G] - gg; b = Bmp[j + B] - bb;
    ...
    if (allpos != null)
    {
        allpos[ok - 1] = (uint)(((sy + y) << 16) | (sx + x));
        if (dir == 0) arr[ok - 1] = (uint)(err1 - e1);
        if (ok >= allpos_max) goto Return1;
    }
    NoMatch:
    if (dir == 9) goto NextSpiral;
    goto NextRaster;
    //----------------------
    Return1:
    if (allpos == null || w * h == 1)
        return ok;
    ...
    if (dir == 0)
    {
        for (i = 1; i < ok; i++)
        {
            uint sk = arr[i], sv = allpos[i];
            for (j = i - 1; j >= 0 && arr[j] > sk; j--)
            {
                arr[j + 1] = arr[j]; allpos[j + 1] = allpos[j];
            }
            arr[j + 1] = sk; allpos[j + 1] = sv;
        }
    }
    ...
    for (i = 0; i < k; i++)
    {
        c1 = allpos[i]; x1 = (int)(c1 & 0xFFFF); y1 = (int)(c1 >> 16);
        for (j = 0; j < ok; j++)
        {
            c2 = allpos[j]; x = (int)(c2 & 0xFFFF) - x1; y = (int)(c2 >> 16) - y1;
...
// GetPicHash's inner loop. FindText.ahk called ntdll\RtlComputeCrc32 once per pixel row and
// combined the rows with hash*31; ntdll is Windows-only, and a script-level loop is not an option
// (a full-screen hash is millions of pixels per WaitChange poll). This is that same CRC-32 â€”
// the ordinary reflected-0xEDB88320 one, which is exactly what RtlComputeCrc32(0, â€¦) computes â€”
// so the row loop and the combination below are line-for-line the original's.
[Keysharp.Runtime.Static] public static unsafe long PicHash(object @this, long bmpPtr, long strideArg, long xArg, long yArg, long wArg, long hArg)
{
    int Stride = (int)strideArg, x = (int)xArg, y = (int)yArg, w = (int)wArg, h = (int)hArg;
    byte* p = (byte*)bmpPtr + (long)y * Stride + (long)x * 4;
    int bytes = w * 4;
    ulong hash = 0;
    ...
    for (int row = 0; row < h; row++, p += Stride)
    {
        ...
        hash = (hash * 31 + (crc ^ 0xFFFFFFFFu)) & 0xFFFFFFFFu;
    }
    return (long)hash;
}

// QPC()'s clock. QueryPerformanceFrequency/Counter are Win32; Stopwatch reads the same monotonic
// hardware counter and exists everywhere. Milliseconds with sub-ms resolution, as before.
[Keysharp.Runtime.Static] public static double QPC(object @this) =>
...
__New()
{
  ; PORTED: the screenshot is an Image's pixel Buffer, not a DIB section. Data holds the Buffer
  ; (which owns the memory), Scan0 its address so the raw NumGet/NumPut sites are unchanged, and
  ; hBM/oldzw/oldzh are gone.
  this.bits:={ Data: 0, Scan0: 0, Stride: 0, zx: 0, zy: 0, zw: 0, zh: 0 }
...
__Delete()
{
  ; PORTED: nothing to release by hand — the screenshot is a Buffer, not a DIB section.
}
...
; --- PORTED: the matcher now calls the #CSharp FindTextClass.PicFind instead of DllCall'ing MCode. ---
PicFind(ini, j, dir, sx, sy, sw, sh)
{
  ...
  ok:=(!ini.bits.Scan0 || mode<1 || mode>5) ? 0
    ...
    , ini.bits.zw, ini.bits.zh
    , sx, sy, sw, sh
    , ini.ss, ini.s1, ini.s0
    , text, w, h
    , Floor(Abs(err1)*1024), Floor(Abs(err0)*1024)
    , (err1<0||err0<0), ini.allpos, ini.allpos_max
    , Floor(w*ini.zoomW), Floor(h*ini.zoomH))
  return ok
}
...
PicInfo(text)
{
  if !InStr(text, "$")
    return
  static info:=Map(), bmp:=[]
  ; PORTED: the key was CRC-32'd via ntdll for long strings; the string itself is a fine Map key
  ; and is one less platform dependency.
  key:=(v:=Trim(text,"|"))
  if info.Has(key)
    return info[key]
  ...
  if (mode=5)
  {
    ...
    {
      ; PORTED: LoadPicture + CreateDIBSection + CopyHBM + RtlMoveMemory collapse into one
      ; GetPixelData call. The buffer layout is unchanged -- w*h*4 pixel bytes followed by the
      ; color table -- because GetPixelData writes exactly w*h*4 bytes at the START of a buffer
      ; that may be larger, which is what lets the pixels and the colors share one allocation
      ; exactly as before. That in turn is why the matcher can still find `cors` at text+w*h*4.
      if !IsObject(img:=this.LoadPic(v))
      {
        MsgBox "Can't Load Picture ! " v, "Tip", 4096
        return
      }
      this.GetBitmapWH(img, &w, &h)
      if (w<1 || h<1)
        return
      arr:=StrSplit(color "/", "/"), arr.Pop(), n:=arr.Length
      bmp.Push(buf:=Buffer(w*h*4 + n*2*4)), v:=buf.Ptr, p:=v+w*h*4-4
      img.GetPixelData(4, buf), img.Dispose()
      color:=Trim(arr[1],"-")
      For k1,v1 in arr
  ...
  else
  {
    ...
    if (w<1 || h<1 || StrLen(v)!=w*h)
      return
    arr:=StrSplit(color "/", "/"), arr.Pop(), n:=arr.Length
    ; PORTED: "CP0" (the system ANSI page) is not the same thing on every OS; UTF-8 is spelled out
    ; instead. For a string of only '0' and '1' both are one byte per character, so the layout is
    ; identical -- the bit data still occupies exactly w*h bytes with the color table after it.
    , bmp.Push(buf:=Buffer(StrPut(v, "UTF-8") + n*2*4))
    , StrPut(v, buf.Ptr, "UTF-8"), v:=buf.Ptr, p:=v+w*h-4
...
; PORTED: the <FindPic> source. FindText.ahk used LoadPicture(), which also accepts "HBITMAP:*handle".

LoadPic(spec)
{
  ; An Image passes straight through: several call sites that used to build an "HBITMAP:*<handle>"
  ; string now simply hand the Image along, and this is where they converge.
  if IsObject(spec)
    return spec
  Try
  {
    if RegExMatch(spec, "i)^\s*HBITMAP:\*?(\w+)\s*$", &r)
      return Image.FromBitmap(this.Floor(r[1]))
    return Image.FromFile(spec)
  }
  return 0
}
...
VirtualScreen()
{
  ...
  Loop MonitorGetCount()
  {
    ...
    if (A_Index=1)
      ...
      x1:=Min(x1,l), y1:=Min(y1,t), x2:=Max(x2,r), y2:=Max(y2,b)
  }
  return {X:x1, Y:y1, Width:x2-x1, Height:y2-y1}
}

; PORTED: the screenshot is an Image's pixel Buffer instead of a screen-sized DIB blitted into by
; BitBlt/GetDCEx/PrintWindow. The CONTRACT is deliberately the original's, because every caller
; depends on it: x/y/w/h go in and come back as SCREEN coordinates clipped to what was captured,
; and zx/zy/zw/zh describe the capture, so the callers' "x-=zx, y-=zy" still converts to buffer
; coordinates exactly as before.
;
; One pixel of margin is grabbed around the request because Gray Difference mode compares every
; pixel with its neighbours; without it the border row/column would compare against zeros.
  ...
  , ScreenShot:=1, &zx:=0, &zy:=0, &zw:=0, &zh:=0)
{
  ...
  if (ScreenShot)
  {
    ...
    , cx:=Max(x-1, vs.X), cy:=Max(y-1, vs.Y)
    , cw:=Min(x+w+1, vs.X+vs.Width)-cx, ch:=Min(y+h+1, vs.Y+vs.Height)-cy
    if (cw>=1 && ch>=1)
    {
      Try
      {
        ; BindWindow() is the only reason this is not a plain Image.FromRect: when a window is
        ; bound its own pixels are painted over the screen capture so it can be searched while it
        ; is occluded. this.bind.id is 0 whenever nothing is bound, which is the common path.
        img:=(this.bind.id ? this.BindCapture(cx, cy, cw, ch) : Image.FromRect(cx, cy, cw, ch))
        ; Reuse the previous capture's Buffer when it is big enough: a screenshot loop would
        ; otherwise allocate, and leave for the GC, one buffer per frame. GetPixelData writes
        ; exactly Width*Height*4 bytes at the start and returns the buffer it was given.
        , need:=img.Width*img.Height*4
        , bits.Data:=img.GetPixelData(4
          , (IsObject(bits.Data) && bits.Data.Size>=need) ? bits.Data : Buffer(need))
        , bits.zx:=img.OriginX, bits.zy:=img.OriginY
        , bits.zw:=img.Width, bits.zh:=img.Height
        , bits.Stride:=bits.zw*4
        ; Scan0 keeps its original meaning, the address of the top-left pixel, so every
        ; NumGet(bits.Scan0 + y*Stride + x*4) elsewhere works unchanged. bits.Data holds the Buffer
        ; itself so it stays alive for as long as Scan0 is used.
        , bits.Scan0:=bits.Data.Ptr
        , img.Dispose()
      }
    }
  }
  zx:=bits.zx, zy:=bits.zy, zw:=bits.zw, zh:=bits.zh
  ; Clip the requested rectangle to the capture, in screen coordinates. These are the original's
  ; two lines, with zx/zy now the capture's origin rather than the virtual screen's.
  w:=Min(x+w,zx+zw), x:=Max(x,zx), w-=x
  , h:=Min(y+h,zy+zh), y:=Max(y,zy), h-=y
  return bits
}

; PORTED: the bitmap helpers below. Everywhere FindText.ahk passed an HBITMAP handle it now passes
; a KS Image object, so the call sites keep their shape and only these bodies changed. The
; parameters keep their original names (hBM, hBM1, ...) for the same reason.
;
; CopyBits() is gone: it copied raw DIB rows via MFCopyImage/RtlMoveMemory and had no callers left
; once the screenshot became an Image.

; Replaces bits' capture buffer with an image the caller supplies (ShowPic's job). GetBitsFromScreen
; owns that buffer normally; this is the one place something else installs one.

UpdateBits(bits, img, zx, zy)
{
  need:=img.Width*img.Height*4
  , bits.Data:=img.GetPixelData(4
    , (IsObject(bits.Data) && bits.Data.Size>=need) ? bits.Data : Buffer(need))
  , bits.Scan0:=bits.Data.Ptr
  , bits.zx:=zx, bits.zy:=zy
  , bits.zw:=img.Width, bits.zh:=img.Height
  , bits.Stride:=bits.zw*4
}
...
{
  ; A blank ARGB canvas. bpp and a negative height (the DIB's top-down flag) no longer mean
  ; anything -- an Image is always top-down 32bpp -- but the signature is kept for its callers.
  return Image.Create(Abs(w), Abs(h))
}

GetBitmapWH(hBM, &w, &h)
{
  w:=hBM.Width, h:=hBM.Height
}

CopyHBM(hBM1, x1, y1, hBM2, x2, y2, w, h, Clear:=0)
{
  if (w<1 || h<1 || !IsObject(hBM1) || !IsObject(hBM2))
    return
  ; Clear was a MERGECOPY blit that flattened the destination first; Image.DrawImage already
  ; overwrites the destination rectangle, so it needs nothing extra.
  hBM1.DrawImage(hBM2.Copy().Crop(x2, y2, w, h), x1, y1, w, h)
}
...
DrawHBM(hBM, lines)
{
  ...
  if IsObject(v)
    hBM.FillRect(v[1], v[2], v[3], v[4], v[5])
}
...
; Retained for compatibility with FindText.ahk. Keysharp captures without the pointer.
...
; PORTED. Image.GetPixelData(4) hands back R,G,B,A; a Windows DIB was B,G,R,A. So a little-endian
; NumGet("uint") over the capture reads 0xAABBGGRR where FindText.ahk's code expects 0xAARRGGBB.
; Swapping the two outer bytes converts either way (it is its own inverse), which is why the four
; places that read or write a whole pixel as a uint call this and are otherwise unchanged. The
; matcher itself does not need it: the #CSharp block reads the channels at their own offsets.
...
GetColor(x, y, fmt:=1)
{
  ...
  ? this.SwapRB(NumGet(bits.Scan0+y*bits.Stride+x*4,"uint")) : 0xFFFFFF
...
SetColor(x, y, color:=0x000000)
{
  ...
  if (x>=0 && x<zw && y>=0 && y<zh && bits.Scan0)
    NumPut("uint", this.SwapRB(color), bits.Scan0+y*bits.Stride+x*4)
}
...
; Return Association array {text:Text, X:X, Y:Y, Width:W, Height:H}
...
Ocr(ok, offsetX:=20, offsetY:=20, overlapW:=0)
{
  ...
  return {text:ocr_Text, X:ocr_X, Y:min_Y, Width:min_X-ocr_X, Height:max_Y-min_Y}
}
...
MouseTip(x:="", y:="", w:=10, h:=10, d:=3)
{
  if (x="")
  {
    ; PORTED: was GetCursorPos. Screen coords, matching what RangeTip below expects.
    bak:=A_CoordModeMouse, CoordMode("Mouse", "Screen")
    MouseGetPos(&x, &y)
    CoordMode("Mouse", bak)
  }
  Loop 4
  ...
  , ScreenShot:=1, &zx:=0, &zy:=0, &zw:=0, &zh:=0)
{
  ...
  if (w<1 || h<1 || !bits.Data)
    return
  ; PORTED: crop straight out of the capture Buffer instead of blitting between two DIBs.
  ; GetBitsFromScreen already returned x/y in buffer pixels, so no zx/zy subtraction here.
  hBM:=Image.FromBuffer(bits.Data, bits.zw, bits.zh, 4).Crop(x-zx, y-zy, w, h)
  hBM.SetOrigin(x, y)
  return hBM
}
...
{
  ; PORTED: an Image replaces the DIB, so writing the BMP header by hand and stuffing a packed DIB
  ; onto the clipboard through GlobalAlloc/SetClipboardData both collapse into Image/KS calls. The
  ; file's extension now picks the format (the original always wrote a .bmp).
  img:=IsObject(hBM_or_file) ? hBM_or_file.Copy() : this.LoadPic(hBM_or_file)
  if !IsObject(img)
    return
  if (w!=0 && h!=0)
    img.Crop(x, y, w, h)
  if (!file)
  {
    ; Clipboard.Image takes the Image directly. This used to round-trip through a temp PNG because the
    ; old CopyImageToClipboard only accepted a filename.
    Clipboard.Image:=img
  }
  else
  {
...
ShowPic(file:="", show:=1, &x:="", &y:="", &w:="", &h:="")
{
  ...
  if !IsObject(hBM:=this.LoadPic(file))
    return
  this.GetBitmapWH(hBM, &w, &h)
  bits:=this.GetBitsFromScreen(&_x:=0,&_y:=0,&_w:=0,&_h:=0,0,&x,&y,&zw,&zh)
  ; PORTED: FindText.ahk grew the full-screen DIB and blitted the picture into its top-left, so the
  ; surrounding screen pixels survived. The capture buffer is now exactly the picture, which is what
  ; every caller here actually wanted; a script relying on the surrounding pixels would notice.
  this.UpdateBits(bits, hBM, x, y)
  Try hBM.Dispose()
  if (show)
...
; PORTED: instead of blitting onto the control's DC (which the next WM_PAINT would erase anyway),
; hand the Picture control the image. x1/y1 are always 0 at both call sites, so the destination
; offset is not reproduced; a non-zero one would need a padded canvas.
...
BitmapToWindow(hwnd, x1, y1, hBM, x2, y2, w, h)
{
  ...
  if !IsObject(ctrl)
    return
  ; Whether a copy was made is remembered in a flag rather than re-derived by comparing img against
  ; hBM afterwards. Two reasons, and the second one cost a real bug: variable names are
  ; case-insensitive, so a local named `hbm` IS this function's `hBM` parameter -- assigning the
  ; bitmap handle to it left the later comparison testing an Image against an integer, which is
  ; always unequal, so every call disposed the CALLER's canvas (Pic_hBM, GridImg).
  cropped:=(x2 || y2 || w!=hBM.Width || h!=hBM.Height)
  img:=cropped ? hBM.Copy().Crop(x2, y2, w, h) : hBM
  ctrl.Value:="HBITMAP:*" . img.ToBitmap()
  if (cropped)
    Try img.Dispose()
}
...
GetPicHash(x1:=0, y1:=0, x2:=0, y2:=0, ScreenShot:=1)
{
  ...
  if (w<1 || h<1 || !bits.Scan0)
    return 0
  ; PORTED: the per-row ntdll\RtlComputeCrc32 loop moved into the #CSharp block. Same CRC-32 and the
  ; same hash*31 row combination; a script-level loop is not viable at millions of pixels per poll.
  ; The VALUE differs from FindText.ahk's, because it hashes the pixel bytes and ours are RGBA where
  ; the DIB was BGRA. It is only ever compared against itself (WaitChange), so that is harmless.
  return FindTextClass.PicHash(bits.Scan0, bits.Stride, x, y, w, h)
}
...
WindowToScreen(&x, &y, x1, y1, id:="")
{
  if (!id)
    id:=WinGetID("A")
  WinGetPos(&wx, &wy, , , id)   ; PORTED: was GetWindowRect
  , x:=x1+wx, y:=y1+wy
}
...
ClientToScreen(&x, &y, x1, y1, id:="")
{
  if (!id)
    id:=WinGetID("A")
  WinGetClientPos(&cx, &cy, , , id)   ; PORTED: was ClientToScreen
  , x:=x1+cx, y:=y1+cy
}
...
Class Thread
{
  ...
  Exec(s, Ahk:="", args:="")
  {
    ...
    ; PORTED, two changes, both because the originals are Windows-only:
    ;   * the injected first line was DllCall("SetWindowText", ..., A_ScriptHwnd, ...). WinSetTitle
    ;     does the same thing through the cross-platform window layer.
    ;   * the ComObject("WScript.Shell") branch that piped the script through StdIn is gone. The
    ;     class lives behind #if WINDOWS, so on Linux/macOS the name does not exist at all. What was
    ;     its Catch fallback -- write a temp file and Run it -- is the whole implementation now; it
    ;     produced an identical result on Windows anyway, so nothing is lost but a code path.
    f:=A_Temp . (InStr(A_Temp, "\") ? "\" : "/") . "~ahk.tmp"
    s:="`nTry WinSetTitle(`"<AHK>`", `"ahk_id `" A_ScriptHwnd)`n`n`n" . s
    s:="`nTry FileDelete `"" f "`"`n" . RegExReplace(s, "\R", "`n")
    Try FileDelete f
    FileAppend s, f
  ...
  Clear()
  {
    Try FileDelete A_Temp . (InStr(A_Temp, "\") ? "\" : "/") . "~ahk.tmp"
    SetTimer(,0)
  }
}
...
QPC()
{
  ; PORTED: was QueryPerformanceFrequency/Counter. Stopwatch is the same monotonic hardware
  ; counter the Win32 pair reads, and it is available on every platform.
  return FindTextClass.QPC()
}
...
ToolTip(s:="", x:="", y:="", num:=1, arg:="")
{
  ...
  fontName:=arg.HasOwnProp("font") ? arg.font : "Consolas"
  ...
  ;-----------------
  r:=bgcolor "|" color "|" fontName "|" size "|" bold "|" trans "|" s
  if (!ini.Has(f) || ini[f]!=r)
  {
    ...
    _Gui.SetFont("c" color " s" size " " bold, fontName)
    _Gui.Add("Text",, s)
    _Gui.Title:=f
    _Gui.Show("Hide")
...
; PORTED: was SendMessage(PBM_SETBARCOLOR) to one Progress control per cell. Now it records the
; cell's colour; Gui("GridPaint") flushes them all to the Picture control in one pass, which is what
; turns 1846 repaints into one. The second parameter is a cell index (see MakeCaptureWindow).

SC(RGB, cell)
{
  this.Gui("SetCell", cell, RGB)
}

; Draws an emoji into a small Image and installs it as the tray icon. Replaces
; TraySetIcon "Shell32.dll", 23 -- that DLL is Windows-only, while an emoji renders on all three
; platforms given the right font. Best-effort: a failure just leaves the default icon in place.

SetTrayEmoji(emoji, size:=64, fontSize:=42)
{
  img:=""
  Try
  {
    fontName:=Font.Emoji.Name
    img:=Image.Create(size, size)
    sz:=img.MeasureText(emoji, "s" fontSize, fontName)
...
Gui(cmd, arg1:="", args*)
{
  ...
  if (!init && init:=1)
  {
    ...
    if (!A_IsCompiled && A_LineFile=A_ScriptFullPath)
    {
      ...
      ; PORTED: was TraySetIcon "Shell32.dll", 23 -- a Windows system DLL with no counterpart
      ; elsewhere. Drawn from an emoji instead, the way demos/Shell.ks does it.
      this.SetTrayEmoji("🔍")
    }
    Critical cri
  ...
  Case "Cancel", "Cancel2":
    WinHide
    return
  ; PORTED: the pixel grid. SetCell records one cell's colour; GridPaint draws every cell into the
  ; grid Image and hands it to the Picture control. Callers set colours freely and paint once.
  Case "SetCell":
    if (Cells.Has(arg1) && Cells[arg1] != args[1])
      Cells[arg1]:=args[1], GridPending[arg1]:=1, GridDirty:=1
    return
  Case "GridPaint":
    if (!GridDirty && arg1!=1)
      return
    GridDirty:=0
    ; One rect per cell, at the cell PITCH, so the 1px background gap between cells survives -- that
    ; grid of gaps is what makes the preview read as pixels rather than flat blocks. (Merging equal
    ; runs into one rect was tried; it paints over the gaps inside a run.)
    ;
    ; Only the cells whose colour actually CHANGED are redrawn. The canvas starts out entirely
    ; WindowColor, which is what every cell starts as, so the opening paint draws nothing at all, and
    ; a single-cell click costs one rect instead of 1846. arg1=1 forces a full repaint.
    if (arg1=1)
    {
      GridPending:=Map()
      Loop GridNW*(GridNH+1)
        GridPending[A_Index]:=1
    }
    For k4 in GridPending
      GridImg.FillRect(Mod(k4-1, GridNW)*(GridCW+1), ((k4-1)//GridNW)*(GridCH+1)
        , GridCW, GridCH, Cells[k4])
    GridPending:=Map()
    this.BitmapToWindow(hGrid, 0, 0, GridImg, 0, 0, GridW, GridH)
    return
  ; Screen point -> 1-based cell index, or 0 when the point is not over the grid. Replaces the
  ; Cid_[hwnd] lookup, which only worked while every cell was its own control. GetClientPos is used
  ; rather than adding a client origin to a control offset: those live in different coordinate
  ; spaces on a HiDPI screen (see the Gui coordinate notes).
  Case "GridCell":
    if !IsObject(GridCtrl)
      return 0
    GridCtrl.GetClientPos(&gx, &gy, &gw, &gh)
    mx:=arg1-gx, my:=args[1]-gy
    if (mx<0 || my<0 || mx>=gw || my>=gh)
      return 0
    ; Grid* rather than w/h/nW/nH for the same reason GridPaint uses them -- see MakeCaptureWindow.
    k3:=mx//(GridCW+1), k4:=my//(GridCH+1)
    if (k3>=GridNW || k4>GridNH)
      return 0
    return k4*GridNW+k3+1
  Case "MakeCaptureWindow":
    ...
    Tab.UseTab(1)
    ; PORTED: one Picture control drawn from an Image, where FindText.ahk used nW*(nH+1) = 1846
    ; Progress controls tinted with PBM_SETBARCOLOR (the Win32 message SC() sent). Creating 1846
    ; controls cost ~10 s here. C_[i] keeps its role as the token the colour sites hand to SC(),
    ; except it is now the cell INDEX rather than an hwnd, so those sites are unchanged; SC() records
    ; the colour and GridPaint() draws the whole grid in one pass.
    C_:=Map(), Cells:=Map(), GridPending:=Map(), GridDirty:=0
    , nW:=71, nH:=25, w:=h:=12, pW:=nW*(w+1)-1, pH:=(nH+1)*(h+1)-1
    ; The grid's geometry is SNAPSHOT here, because nW/nH/w/h/pW/pH are shared scratch that the rest
    ; of Gui() reassigns: CaptureUpdate sets nW/nH to the capture REGION size, PicShow sets w/h to a
    ; scroll range, MakeMainWindow recomputes pW. FindText.ahk only ever read them here, while it was
    ; creating the 1846 cell controls, so reuse was harmless; painting reads them every time, so it
    ; must not. (Symptom when it did: one FillRect the size of the whole canvas -> a flat block.)
    , GridNW:=nW, GridNH:=nH, GridCW:=w, GridCH:=h, GridW:=pW, GridH:=pH
    id:=_Gui.Add("Pic", "w" pW " h" pH), hGrid:=id.Hwnd, GridCtrl:=id
    , GridImg:=Image.Create(pW, pH, WindowColor)
    Loop GridNW*(GridNH+1)
      C_[A_Index]:=A_Index, Cells[A_Index]:=WindowColor
    G_.Call("GridPaint")
  ...
  Case "Capture", "CaptureS":
    ...
    For k,v in Pics   ; PORTED: screenshots are Images now; the GC releases them.
      Try k.Dispose()
    ...
    if (Event="OK")
    {
      ...
      ; The copyable block runs from FindText(args*) to the end of the class, exactly as in
      ; FindText.ahk. It must START at FindText(args*) -- an earlier version began at the #import
      ; header, which sits BELOW that wrapper, so pasted code called a FindText() it never defined
      ; ("This global variable appears to never be assigned a value: FindText"). The #import and
      ; #CSharp blocks now live inside the class, so this one span carries the matcher too; without
      ; it the paste has no FindTextClass.PicFind.
      re:="i)\nFindText\(args\*\)[\s\S]*?Script_End[(){}\s]+}"
  ...
  Case "CaptureUpdate":
    ...
    if (nW>0 && nH>0 && bits.Scan0)
    {
      ...
      Loop nH + 0*(k:=0)
      Loop nW + 0*(p+=j)
        show[++k]:=1, cors[k]:=this.SwapRB(NumGet(p+=4,"uint"))
    }
  ...
  Case "PicUpdate":
    ...
    Try (!i) && hBM_old.Dispose()   ; PORTED: was DeleteObject
  ...
  Case "PicShow":
    ...
    return
  Case "PicShowOK":
    ; PORTED: Pic_hBM is an Image now, not an HBITMAP handle.
    this.BitmapToWindow(hPic, 0, 0, Pic_hBM, 0, 0, Pic_w, Pic_h)
    return
  Case "Reset":
    G_.Call("CaptureUpdate")
  ...
  Case "SavePic":
    ...
    this.ScreenShot(), this.ShowPic(hBM)   ; PORTED: hBM is an Image now
  ...
  Case "RepColor", "CutColor":
    ...
    if (tx:=Mod(k-1,nW)-dx)>=0 && tx<71 && (ty:=(k-1)//nW-dy)>=0 && ty<25
      this.SC(c, C_[ty*71+tx+1]), G_.Call("GridPaint")
    return
  Case "RepL":
  ...
  Case "LButtonDown":
    ...
    ; PORTED: the grid is one control now, so the cell comes from the pointer's position in it
    ; rather than from Cid_[hwnd]. The old -1 branch nudged the mouse to escape the backing Text
    ; control that sat behind the cells; there is nothing to escape any more.
    if !(k5:=this.Gui("GridCell", k1, k2))
      return
    if (k5>71*25)
    {
      k1:=nW*nH+dx+(k5-71*25)
      this.SC(((show[k1]:=!show[k1])?0xFF0000:0xFFFFAA), k5)
      G_.Call("GridPaint")
      return
    }
    ...
    if (Modify && bg!="" && show[k1])
      this.SC(((ascii[k1]:=!ascii[k1])?0:0xFFFFFF), k5), G_.Call("GridPaint")
    else
    {
